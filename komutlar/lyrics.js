...
exports.run = async (Bastion, message, args) => {
  try {
    ...
    if (!song) {
      ...
      return message.reply("**Doğru Kullanım**: f!lyrics <müzik>")
    }
    ...
    if (response.message.header.status_code === 200) {
      message.channel.send({
        embed: {
          ...
          description: response.message.body.lyrics.lyrics_body.replace('******* Bu söz, ticari kullanım için DEĞİLDİR *******', `Lyricsin tamamını buradan bulabilirsin: [musixmatch.com](${response.message.body.lyrics.backlink_url} 'Musixmatch')`),
          footer: {
